...
<section className="section release-reading"><div className="shell"><div className="section-heading"><p className="eyebrow">Read online</p><h2>Three routes. One story.</h2><p>Read all three routes in Japanese and MAO English, with both the Original and All-ages editions available.</p></div><div className="chapter-grid">{routes.map(([n,title,id])=><a className="chapter-card" key={id} href={'script/?section='+id}><span className="chapter-number">{n}</span><div><h3>{title}</h3><p>Available now</p></div></a>)}</div><a className="text-link" href="script/">Open the script browser <span aria-hidden="true">→</span></a></div></section>
