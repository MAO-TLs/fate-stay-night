...
<p className="compatibility">Réalta Nua + restored scenes · Japanese + MAO English · {scriptCount.toLocaleString()} scripts</p></div><div aria-hidden="true"/></div></section>
...
<section className="section release-reading"><div className="shell"><div className="section-heading"><p className="eyebrow">Read online</p><h2>Three routes. One story.</h2><p>Read all three routes, their epilogues, and the restored H-scene versions in Japanese and MAO English.</p></div><div className="chapter-grid">{routes.map(([n,title,id])=><a className="chapter-card" key={id} href={'script/?section='+id}><span className="chapter-number">{n}</span><div><h3>{title}</h3><p>Available now</p></div></a>)}</div><a className="text-link" href="script/">Open the script browser <span aria-hidden="true">→</span></a> <a className="text-link" href="script/?section=last-episode">Read Last Episode <span aria-hidden="true">→</span></a></div></section>
