export function SiteNav({currentPage}: {currentPage: "release" | "script"}) {
 ...
 <div className="nav-links">{([['release',releaseHref,'Release'],['script',scriptHref,'Script']] as const).map(([id,href,label])=><a key={id} href={href} aria-current={id===currentPage?'page':undefined}>{label}</a>)}<a href="https://github.com/MAO-TLs/fate-stay-night">GitHub</a></div></nav>;
}
