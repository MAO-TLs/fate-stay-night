...
export function ScriptReader() {
  ...
  useEffect(() => {
    fetch(`../data/script/index.json?v=${readerDataRevision}`).then(r => {
      ...
      setShowAuditFindings(chosenEdition === "original" && params.get("errors") === "mirror-moon");
  ...
  return <section className="reader-shell shell compact">
    <div id="reader-controls" className="reader-controls-anchor" aria-hidden="true"/><div className="reader-controls">
      ...
      {edition === "original" && <label className="comparison-toggle"><input type="checkbox" checked={showMirrorMoon} onChange={event => { setShowMirrorMoon(event.target.checked); if (!event.target.checked) setShowAuditFindings(false); }}/><span>Display mirror moon for comparison</span><small>Classic edition only</small></label>}
      {edition === "original" && <label className="comparison-toggle comparison-toggle-errors"><input type="checkbox" checked={showAuditFindings} onChange={event => { setShowAuditFindings(event.target.checked); if (event.target.checked) setShowMirrorMoon(true); }}/><span>Display confirmed mirror moon errors</span><small>{showAuditFindings ? `${auditFindings.length.toLocaleString()} findings in this script` : "Source-audited notes"}</small></label>}
    </div>
...
function AuditErrorText({text, findings, activeFindingId, onToggleFinding}: {text: string; findings: AuditFinding[]; activeFindingId: string; onToggleFinding: (id: string) => void}) {
  ...
  for (const {finding, range: {start, end}} of positioned) {
    ...
    fragments.push(<button className="todokanai-error-trigger" type="button" key={finding.id} aria-expanded={activeFindingId === finding.id} onClick={() => onToggleFinding(finding.id)}>{highlight}<span className="todokanai-error-preview" role="tooltip"><strong>{categoryLabel(finding.category)}</strong><span>{finding.explanation}</span></span></button>);
  ...
  return <><p className="todokanai-annotated-text">{fragments}</p>{unpositioned.length ? <div className="todokanai-error-fallbacks">{unpositioned.map(finding => <button type="button" key={finding.id} aria-expanded={activeFindingId === finding.id} onClick={() => onToggleFinding(finding.id)}>View {categoryLabel(finding.category).toLowerCase()} note</button>)}</div> : null}{active ? <aside className="todokanai-error-note" id={`error-note-${active.id}`} aria-label={`mirror moon error note for ${active.ref}`}><header><div><span className="todokanai-error-category">{categoryLabel(active.category)}</span><span className="todokanai-error-severity">{active.status === "confirmed_source_variant" ? "source variant" : "confirmed"}</span></div><button type="button" onClick={() => onToggleFinding(active.id)} aria-label="Close error note">Close</button></header><code>{active.id}</code><div className="todokanai-error-evidence"><span>Japanese</span><p lang="ja">{active.evidenceJa}</p></div><div className="todokanai-error-evidence"><span>Governing reading</span><p>{active.governingReading}</p></div><p className="todokanai-error-explanation">{active.explanation}</p>{active.dossiers.length ? <div className="todokanai-error-actions">{active.dossiers.map(id => <a href={`../audit/#dossier-${encodeURIComponent(id)}`} key={id}>Open related dossier →</a>)}</div> : null}</aside> : null}</>;
}
