...
type Page = {ref: string; ja: string; en: string};
...
const routeNames: Record<string, string> = {prologue: "Prologue", fate: "Fate", ubw: "Unlimited Blade Works", hf: "Heaven's Feel"};
...
export function ScriptReader() {
  ...
  const resultStatus = scope === "script"
    ...
    : !corpusQuery.trim() ? `${index?.pageCount.toLocaleString() ?? "25,507"} passages ready to search` : !concordance ? "Loading complete concordance…" : `${corpusMatches.length.toLocaleString()} matching passages`;
  ...
  return <section className="reader-shell shell compact">
    <div className="reader-controls" id="reader-controls">
      ...
      <div className="control"><label htmlFor="script">Script</label><div className="script-picker"><button onClick={() => move(-1)} disabled={scope === "corpus" || currentPosition <= 0} aria-label="Previous script">←</button><select id="script" value={selectedId} disabled={scope === "corpus"} onChange={event => setSelectedId(event.target.value)}>{routeScripts.map(item => <option key={item.id} value={item.id}>{item.title} · {item.pages.toLocaleString()} passages</option>)}</select><button onClick={() => move(1)} disabled={scope === "corpus" || !index || currentPosition >= index.scripts.length - 1} aria-label="Next script">→</button></div></div>
    ...
    {!error && scope === "script" && data && <><div className="script-meta"><h2>{routeNames[data.route]} · {data.title}</h2><p>{data.pages.length.toLocaleString()} source passages</p></div>{visiblePages.length ? <div className="script-lines" id="script-results">{visiblePages.map(page => <ScriptLine key={page.ref} id={page.ref} page={page}/>)}</div> : <p className="script-empty">No passages match this search.</p>}<a className="back-to-controls" href="#reader-controls">Back to controls ↑</a></>}
    {!error && scope === "corpus" && !corpusQuery.trim() && <div className="concordance-prompt"><p className="eyebrow">Complete corpus</p><h2>Search every route.</h2><p>Enter Japanese or English above to search all 25,507 aligned passages.</p></div>}
    {!error && scope === "corpus" && corpusQuery.trim() && concordance && <><div className="script-meta concordance-meta"><div><p className="eyebrow">Complete corpus</p><h2>Search results</h2></div><p>{corpusMatches.length.toLocaleString()} matches</p></div><div className="concordance-route-filter">{[["all","All sections"], ...Object.entries(routeNames)].map(([id,label]) => <button className={corpusRoute === id ? "is-active" : ""} key={id} onClick={() => setCorpusRoute(id)}>{label}<span>{id === "all" ? corpusMatches.length : concordance.filter(x => x.route === id && `${x.ja}\n${x.en}`.toLocaleLowerCase().includes(corpusQuery.toLocaleLowerCase())).length}</span></button>)}</div><div className="concordance-results" id="concordance-results">{corpusMatches.slice(0,resultLimit).map(page => <article className="concordance-hit" key={`${page.scriptId}-${page.ref}`}><a className="concordance-hit-link" href={`?script=${page.scriptId}#${page.ref}`}><span>{routeNames[page.route]} · {page.title}</span><code>{page.ref}</code><strong>Open script →</strong></a><div className="concordance-hit-grid"><div className="line-cell line-ja"><span className="speaker speaker-ja">Japanese</span><p lang="ja">{displayJapanese(page.ja) || "—"}</p></div><div className="line-cell line-en"><span className="speaker">MAO English</span><p>{page.en || "—"}</p></div></div></article>)}</div>{corpusMatches.length > resultLimit && <p className="script-status">Showing the first {resultLimit} matches. Refine the search to narrow the result set.</p>}</>}
  </section>;
}

function ScriptLine({id, page}: {id: string; page: Page}) {
  return <article className="script-line" id={id} tabIndex={-1}><a className="line-ref" href={`#${id}`} aria-label={`Link to ${id}`}>{id.replace("page", "")}</a><div className="line-cell line-ja"><span className="speaker speaker-ja">Japanese</span><p lang="ja">{displayJapanese(page.ja) || "—"}</p></div><div className="line-cell line-en"><span className="speaker">MAO English</span><p>{page.en || "—"}</p></div></article>;
}
